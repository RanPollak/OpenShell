...
Gateways expose `/healthz` for process liveness and `/readyz` for dependency-aware readiness on the health port. `/readyz` reflects the latest result of a background database health check.
