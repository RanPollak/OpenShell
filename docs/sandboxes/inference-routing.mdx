...
- `router` (default): the client-supplied `model` is overwritten with the route's configured model before forwarding. A mismatch is logged but not rejected, so an agent that hardcodes `gpt-4o` keeps working when the operator swaps to a different upstream model. This preserves the historical rewrite.
